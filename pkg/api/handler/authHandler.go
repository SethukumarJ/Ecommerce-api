...
func (cr *AuthHandler) VerifyAccount(c *gin.Context) {
	...
	err = cr.authUsecase.VerifyAccount(c,email, tokenString)
...
func (cr *AuthHandler) UserSignup(c *gin.Context) {
	...
	_, err := cr.userUsecase.CreateUser(c, newUser)
...
func (cr *AuthHandler) CallBackFromGoogle(c *gin.Context) {
	...
	if code == "" {
		...
		if user == (domain.UserResponse{}) {
			user,err = cr.userUsecase.CreateUser(c,newUser)
			log.Println(newUser)
			...
			accesstoken, err := cr.jwtUsecase.GenerateAccessToken(*newUser.ID, newUser.UserName, "user")
			if err != nil {
			...
			refreshtoken, err := cr.jwtUsecase.GenerateRefreshToken(*newUser.ID, newUser.UserName, "user")
			if err != nil {
			...
			refreshtoken, err := cr.jwtUsecase.GenerateRefreshToken(*user.ID, user.UserName, "user")
			if err != nil {
				response := response.ErrorResponse("Failed to generate refresh token", err.Error(), nil)
				c.Writer.Header().Add("Content-Type", "application/json")
				c.Writer.WriteHeader(http.StatusUnauthorized)
...
func (cr *AuthHandler) UserLogin(c *gin.Context) {
	...
	//verify User details
	err := cr.authUsecase.VerifyUser(c,userLogin.Email, userLogin.Password)
...
func (cr *AuthHandler) AdminSignup(c *gin.Context) {
	...
	_,err := cr.adminUsecase.CreateAdmin(c,newAdmin)
...
func (cr *AuthHandler) AdminLogin(c *gin.Context) {
	...
	//verify User details
	err := cr.authUsecase.VerifyAdmin(c,adminLogin.Email, adminLogin.Password)
	...
	refreshtoken, err := cr.jwtUsecase.GenerateRefreshToken(*admin.ID, admin.UserName, "admin")
	if err != nil {
		response := response.ErrorResponse("Failed to generate refresh token", err.Error(), nil)
		c.Writer.Header().Add("Content-Type", "application/json")
		c.Writer.WriteHeader(http.StatusUnauthorized)
